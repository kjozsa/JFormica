...
public class MessageConditionFactory
{
	...
	private static MessageCondition RESPONSE_FILTER_CONDITION = new MessageCondition()
	{
		...
		{
			if( !GENERIC_RESPONSE_CONDITION.test( msg ) )
			{
				return false;
			}
			Response response = (Response) msg;
			if( response.getMessageId().equals( MessageId.EVENT ) )
			{
				return false;
			}
			return true;
	 ...
	 * @return
	 */
	public static <V extends StandardMessage> MessageCondition newResponseCondition( MessageId id, ResponseCode responseCode )
	{
	...
	{
		final MessageCondition complete = MessageConditionFactory.newResponseCondition( MessageId.EVENT,
	...
	private static class ResponseCondition implements MessageCondition
	{
		...
		{
			if( !(testMsg instanceof Response) )
			{
				return false;
			}
			...
			{
				if( !(response.getResponseCode().getCode() == responseCode.getCode()) )
				{
					return false;
				}
			}
