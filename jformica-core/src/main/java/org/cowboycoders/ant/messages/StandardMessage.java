 ...
 *
 */
public abstract class StandardMessage implements MessageDecorator, Messageable
{
	...
	protected StandardMessage( MessageId id, DataElement[] messageElements )
	{
	 ...
	 */
	protected StandardMessage( Message message, MessageId id, ArrayList<DataElement> messageElements )
	{
	 ...
	 * @throws FatalMessageException if there is an error building message
	 */
	protected StandardMessage( Message message, MessageId id, DataElement[] messageElements )
	{
	 ...
	 * {@inheritDoc}
	 */
	protected final void setId( MessageId id )
	{
		message.setId( id );
	}
	...
	@Override
	public final void setStandardPayload( ArrayList<Byte> payload ) throws ValidationException
	{
	 ...
	 */
	@Override
	public final void decode( byte[] buffer ) throws MessageException
	{
		decode( buffer, false );
	}
	...
	protected final void decode( byte[] buffer, boolean noChecks ) throws MessageException
	{
		...
		if( !noChecks )
		{
			...
			{
				throw new MessageException( "Mesage Id does not match that expected for" + " " + this.getClass() );
			}
			if( this.allElementsMustBePresent && getStandardPayload().size() < totalElementLength )
			{
				throw new MessageException( "Insuffucient data for" + " " + this.getClass() );
			}
			validate();
		}
	}
	...
	protected void addOptionalDataElement( DataElement element )
	{
		...
		DataElement[] newElements = Arrays.copyOf( messageElements, messageElements.length + 1 );
	 ...
	 */
	protected void setAndValidateDataElement( DataElement element, int value, int skip ) throws ValidationException
	{
		...
		ValidationUtils.maxMinValidator( minValue,
		                                 maxValue,
		                                 value,
