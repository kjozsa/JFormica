...
public class AddChannelIdMessage extends ChannelMessage {
    ...
    private static DataElement[] additionalElements = new DataElement[]{
            DataElement.DEVICE_NUMBER, DataElement.DEVICE_TYPE, DataElement.TRANSMISSION_TYPE, DataElement.LIST_ID,
    };

    /** companion object we delegate shared methods to */
    private ChannelIdCompanion companion;

    public AddChannelIdMessage(Integer channelNo, int deviceNumber, int deviceType, int transmissionType, int listIndex) {
    ...
    public AddChannelIdMessage(int deviceNumber, int deviceType, int transmissionType, int listIndex) {
        this(0, deviceNumber, deviceType, transmissionType, listIndex);
    }
