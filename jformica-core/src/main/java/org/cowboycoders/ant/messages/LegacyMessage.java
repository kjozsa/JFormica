 ...
 * @author will
 */
public class LegacyMessage extends Message implements ExtendedInformationQueryable, DeviceInfoQueryable, DeviceInfoSettable {
    ...
    public ArrayList<Byte> getStandardPayload() {
        ...
        List<Byte> subList = extendedPayload.subList(DATA_OFFSET, extendedPayload.size());
    ...
    public void setStandardPayload(ArrayList<Byte> payload) throws ValidationException {
        ...
        try {
            ...
            for (int i = 1; i < payload.size() + DATA_OFFSET && i - DATA_OFFSET < AntMesg.MESG_DATA_SIZE - 1; i++) {
    ...
    private void addExtendedElementsToPayload() {
        ...
        for (DataElement element : extendedElements) {
            ...
            try {
                ...
                throw new FatalMessageException("Error initialising extended data", e);
            }
        }
    }
     ...
     */
    private static void checkExtendedFormat(Message message) throws MessageException {
        ...
        if (id != MessageId.EXT_ACKNOWLEDGED_DATA && id != MessageId.EXT_BROADCAST_DATA && id != MessageId.EXT_BURST_DATA) {
     ...
     * limits
     *
     * @param element the {@code DataElements} member to check
     ...
     *
     * @param element       corresponding {@code DataElements}
     ...
     * member
     *
     * @param element the corresponding {@code DataElements} to insert data for
