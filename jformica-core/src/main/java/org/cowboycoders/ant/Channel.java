...
import org.cowboycoders.ant.messages.config.*;
...
public class Channel {
    ...
    private final BroadcastListener<BurstDataMessage> burstListener = new BroadcastListener<BurstDataMessage>() {
        ...
        public void receiveMessage(BurstDataMessage message) {
            ...
            // if timeout
            if (lastBurstTimeStamp != null && (newTimeStamp - lastBurstTimeStamp) >= burstTimeout) {
    ...
    public synchronized <V extends ChannelMessage> void registerRxListener(final BroadcastListener<V> listener, final Class<V> clazz) {
        ...
        BroadcastListener<ChannelMessage> adapter = new BroadcastListener<ChannelMessage>() {
            ...
            public void receiveMessage(ChannelMessage message) {
                ...
                if (!burstFilterCondition.test(message) && listener != burstListener) {
                    return;
                }
    ...
    public synchronized <V extends ChannelMessage> void removeRxListener(final BroadcastListener<V> listener) {
     ...
     */
    public void assign(NetworkKey key, ChannelType type, ExtendedAssignment... extended) {
        assign(key, new ChannelAssignMessage(0, type, extended));
    }

    public void assign(Network network, ChannelType type, ExtendedAssignment... extended) {
     ...
     */
    public void setId(int deviceNumber, int deviceType, int transmissionType, boolean setPairingFlag) {
    ...
    public StandardMessage sendAndWaitForMessage(final ChannelMessage msg,
        ...
        return parent.sendAndWaitForMessage(msg, condition, timeout, timeoutUnit, channelSender, receipt, channelFilterCondition);
    }
    ...
    public synchronized void open() {
        log.info("Opening Channel {}...", getName());
     ...
     */
    public void sendBurst(byte[] data, Long timeout, TimeUnit timeoutUnit) throws
        ...
        try {
            ...
            final MessageCondition completed = MessageConditionFactory.newResponseCondition(MessageId.EVENT,
                    ResponseCode.EVENT_TRANSFER_TX_COMPLETED);
            final MessageCondition failed = MessageConditionFactory.newResponseCondition(MessageId.EVENT,
            ...
            MessageSender massSender = new MessageSender() {
                ...
                public ArrayList<MessageMetaWrapper<? extends StandardMessage>> send(StandardMessage msgIn) {
                    ArrayList<MessageMetaWrapper<? extends StandardMessage>> sentMessages = new ArrayList<>(list.size());
            ...
            try {
                ...
                parent.sendAndWaitForMessage(null, condition, timeout, timeoutUnit, massSender, null, channelFilterCondition);
     ...
     */
    public synchronized void registerBurstListener(BroadcastListener<CombinedBurst> listener) {
     ...
     * @param listener new listener
     */
    public synchronized void removeBurstListener(BroadcastListener<CombinedBurst> listener) {
    ...
    public ChannelStatusResponse requestStatus() throws ChannelError {
        ...
        try {
            response = (ChannelStatusResponse) sendAndWaitForMessage(msg, condition, 1L, TimeUnit.SECONDS, null);
        } catch (InterruptedException | TimeoutException e) {
    ...
    public void updateExclusionInclusionList(int index, ChannelId id) {
        ChannelMessage msg = new AddChannelIdMessage(id.getDeviceNumber(), id.getDeviceType(), id.getTransmissonType(), index);
        MessageCondition condition = MessageConditionFactory.newResponseCondition(msg.getId(), ResponseCode.RESPONSE_NO_ERROR);
        try {
            sendAndWaitForMessage(msg, condition, 1L, TimeUnit.SECONDS, null);
        } catch (InterruptedException | TimeoutException e) {
    ...
    private void handleTimeOutException(Exception e) {
        if (e instanceof InterruptedException) {
            throw new ChannelError("Interrupted whilst waiting for message / reply", e);
        }
