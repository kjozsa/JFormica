package org.cowboycoders.ant.examples.demos.unfinished.scales;

public class DataNotReadyException extends Exception
{

	/**
	 *
	 */
	private static final long serialVersionUID = -7170669734535853233L;

	public DataNotReadyException()
	{
		// TODO Auto-generated constructor stub
	}

	public DataNotReadyException( String message )
	{
		super( message );
		// TODO Auto-generated constructor stub
	}

	public DataNotReadyException( Throwable cause )
	{
		super( cause );
		// TODO Auto-generated constructor stub
	}

	public DataNotReadyException( String message, Throwable cause )
	{
		super( message, cause );
		// TODO Auto-generated constructor stub
	}

	public DataNotReadyException( String message, Throwable cause, boolean enableSuppression, boolean writableStackTrace )
	{
		super( message, cause, enableSuppression, writableStackTrace );
		// TODO Auto-generated constructor stub
	}

}
